  ...
  return (
    <section className="overflow-hidden rounded-xl border border-blue-200 bg-blue-50/50">
      <button type="button" onClick={onToggle} aria-expanded={isOpen} aria-controls="import-instructions" className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500">
        ...
        <span className="flex items-center gap-1 text-sm font-medium text-blue-700">{isOpen ? 'Hide Instructions' : 'Show Instructions'}{isOpen ? <ChevronUp aria-hidden="true" className="h-4 w-4" /> : <ChevronDown aria-hidden="true" className="h-4 w-4" />}</span>
      </button>
      <div className={`grid transition-[grid-template-rows] duration-200 ease-out ${isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
        <div id="import-instructions" aria-hidden={!isOpen} className="overflow-hidden">
          ...
          <div className="mt-4 flex gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
            ...
            <p>Importing will add or update student records. Existing grades for the same student and subject will be overwritten. This action is logged in the audit trail.</p>
          </div>
          </div>
        </div>
      </div>
