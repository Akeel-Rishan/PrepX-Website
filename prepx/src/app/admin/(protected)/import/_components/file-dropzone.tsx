      ...
      <div className="rounded-xl border-2 border-green-300 bg-green-50 p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-green-800">
                Size: {formatSize(selectedFile.size)}, type:{' '}
          ...
          <button
            ...
            onClick={onClear}
            disabled={disabled}
            className="flex shrink-0 items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-green-800 hover:bg-green-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-600 disabled:cursor-not-allowed disabled:opacity-50"
          >
    ...
    <div>
      ...
      >
        <button
          ...
          className="w-full cursor-pointer rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 disabled:cursor-not-allowed"
        >
          <Upload
            aria-hidden="true"
            className={cn(
              'mx-auto mb-3 h-8 w-8',
              error ? 'text-red-500' : isDragging ? 'text-blue-600' : 'text-gray-400'
            )}
          />
          <p
            className={cn(
              'text-sm font-semibold',
              error ? 'text-red-800' : isDragging ? 'text-blue-800' : 'text-gray-800'
            )}
          >
            {disabled
              ? 'Select an examination to enable upload'
              : isDragging
                ? 'Drop to upload'
          ...
          <p className="mt-3 text-xs text-gray-500">Accepted: .xlsx and .csv, up to 10 MB</p>
        </button>
      </div>
      {error && (
