...
export function StudentGradeEditorModal({
  isOpen,
  onClose,
  onSuccess,
  student,
  allSubjects,
  examinationId,
}: StudentGradeEditorModalProps): React.JSX.Element | null {
  const [modalGrades, setModalGrades] = useState<Record<string, string>>({});
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const initialGrades = useMemo(
    () =>
      Object.fromEntries((student?.grades ?? []).map((grade) => [grade.subject_id, grade.grade])),
    [student]
  );
  useEffect(() => {
    if (isOpen && student) {
      setModalGrades(initialGrades);
      setError(null);
    }
  }, [initialGrades, isOpen, student]);
  if (!student) return null;
  const dirtyCount = allSubjects.filter(
    (subject) => (modalGrades[subject.id] ?? '') !== (initialGrades[subject.id] ?? '')
  ).length;
  ...
  async function handleSave() {
    ...
    const changes: GradeChange[] = allSubjects.flatMap((subject) => {
      ...
      return grade === (initialGrades[subject.id] ?? '')
        ? []
        : [{ studentId: student!.id, subjectId: subject.id, grade: grade || null }];
    });
    setIsSaving(true);
    setError(null);
    try {
  ...
  return (
    <Modal
      ...
      title={`Edit Grades: ${student.full_name}`}
      description={`${student.index_number}, ${student.school_name}`}
      size="md"
    >
      <div className="space-y-2">
          ...
          return (
            <div
              ...
              className={cn(
                ...
                missing ? 'border-amber-200 bg-amber-50' : 'border-transparent bg-gray-50'
              )}
            >
              <div className="min-w-0 flex-1">
                ...
                <span
                  className={cn(
                    'ml-2 text-xs',
                    subject.required ? 'text-blue-600' : 'text-gray-500'
                  )}
                >
        ...
        {allSubjects.length === 0 && (
          <p className="py-6 text-center text-sm text-gray-500">
            No active subjects for this examination.
          </p>
        )}
        <div className="mt-2 flex justify-end gap-3 border-t border-gray-100 pt-4">
          ...
          <Button
            ...
            onClick={handleSave}
            loading={isSaving}
            disabled={!allSubjects.length || dirtyCount === 0}
          >
            Save {dirtyCount || ''} Grade{dirtyCount === 1 ? '' : 's'}
          </Button>
        </div>
      </div>
    </Modal>
