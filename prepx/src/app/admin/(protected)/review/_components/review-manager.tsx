  ...
  if (student.status === 'empty')
    return (
      <span className="text-xs text-gray-600">
        {student.missingCount} required subject{student.missingCount === 1 ? '' : 's'} missing
      </span>
    );
...
function Status({ status }: { status: ReviewStudentRow['status'] }): React.JSX.Element {
  if (status === 'complete')
    return (
  ...
  return (
    <span className="flex items-center gap-1.5 text-xs font-medium text-gray-600">
      <CircleDashed className="h-4 w-4" /> No Grades
    </span>
  );
}
...
export function ReviewManager({
  ...
  isReadOnly,
  currentParams,
}: ReviewManagerProps): React.JSX.Element {
  ...
  const pills: Array<{ value: StatusFilter; label: string; count: number }> = [
    {
      ...
      count: data.summary.incomplete + data.summary.empty,
    },
    ...
    <div className="space-y-4">
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full rounded-lg border border-gray-300 py-2 pl-9 pr-3 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
      ...
      {data.students.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-300 bg-white p-12 text-center">
          ...
          <p className="font-semibold text-gray-800">
            {initialStatusFilter === 'needs_attention' && !initialSearch
              ? 'All students have complete grades!'
              : 'No students match this filter.'}
          </p>
          <p className="mt-1 text-sm text-gray-500">
            {initialStatusFilter === 'needs_attention' && !initialSearch
              ? 'The examination is ready for publication review.'
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">
                  ...
                  <th className="px-5 py-3">Missing Required Subjects</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.students.map((student) => (
                  <tr
                    ...
                    className={cn(
                      'hover:bg-gray-50/60',
                      student.status === 'empty' && 'bg-orange-50/30',
                      student.status === 'incomplete' && 'bg-amber-50/20'
                    )}
                  >
                    <td className="px-5 py-4">
                      ...
                      <p className="mt-0.5 text-xs text-gray-500">{student.school_name}</p>
                    </td>
                    <td className="whitespace-nowrap px-5 py-4">
                      <Status status={student.status} />
                    </td>
                    <td className="px-5 py-4">
                      <MissingList student={student} />
                    </td>
